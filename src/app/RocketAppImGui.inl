...
bool trajectorySampleHasHistoricalMetrics(const TrajectorySample& sample) {
    return
        sample.time_s > 0.0 ||
...
double historyDynamicViscosityPaS(double temperature_k) {
...
double historyReynoldsNumber(
    const rocket::SimulationSnapshot& snapshot,
    const rocket::VehicleModel& vehicle) {
    const double mu = historyDynamicViscosityPaS(snapshot.air_temperature_k);
    return
        (snapshot.air_density_kgpm3 *
         std::max(snapshot.relative_air_speed_mps, 0.0) *
         std::max(vehicle.geometry.body_length_m, 0.001)) /
        std::max(mu, 1e-7);
}
...
void drawSimulationHistoryWindowImGui(
    FloatingWindowState& state,
    const SimulationRuntime& runtime,
    const rocket::VehicleModel& vehicle,
    const rocket::SimulationSnapshot& snapshot) {
    primeWindowRect(state.bounds);
    ImGui::Begin("History", nullptr, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoSavedSettings);
    syncWindowRect(state);
    ...
    drawHistoryMetricPlotImGui(
        ...
        [](const TrajectorySample& sample) { return sample.altitude_m; });
    drawHistoryMetricPlotImGui(
        "history-mach",
        "Mach",
        runtime,
        ...
        [](const TrajectorySample& sample) { return sample.mach_number; });
    drawHistoryMetricPlotImGui(
        "history-q",
        "Dynamic Pressure q",
        runtime,
        ...
        [](const TrajectorySample& sample) { return sample.dynamic_pressure_pa; });
    drawHistoryMetricPlotImGui(
        "history-rho",
        "Density rho",
        runtime,
        ...
        [](const TrajectorySample& sample) { return sample.air_density_kgpm3; });
    drawHistoryMetricPlotImGui(
        "history-p",
        "Static Pressure P",
        runtime,
        ...
        [](const TrajectorySample& sample) { return sample.static_pressure_pa; });
    drawHistoryMetricPlotImGui(
        "history-p0",
        "Total Pressure P0",
        runtime,
        ...
        [](const TrajectorySample& sample) { return sample.total_pressure_pa; });
    drawHistoryMetricPlotImGui(
        "history-re",
        "Reynolds",
        runtime,
        historyReynoldsNumber(snapshot, vehicle),
        std::format("{:.2e}", historyReynoldsNumber(snapshot, vehicle)),
        ImVec4(0.93f, 0.63f, 0.98f, 1.0f),
        [](const TrajectorySample& sample) { return sample.reynolds_number; });
...
void renderDearImGuiUi(
    ...
    if (app_state.workspace == Workspace::Modeling) {
        ...
        if (!runtime.keyframe_preview_active) {
            drawSimulationHistoryWindowImGui(
                app_state.layout.sim_keyframe,
                runtime,
                vehicle,
                simulation_snapshot);
        }
